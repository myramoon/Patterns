...
read -p "Enter a password [minimum 8 characters and at least 1 uppercase,1 digit,special character] : " input4
...
if [[ ${#input4} -ge 8 && "$input4" == *[A-Z]* && "$input4" == *[0-9]* && "$input4" == *[[:punct:]]*  && "$input4" == [A-Za-z0-9]* ]]
then
        password=1
else
        password=0
